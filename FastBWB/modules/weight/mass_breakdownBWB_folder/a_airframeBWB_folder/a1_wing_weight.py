"""
Estimation of wing weight - A1 COMPONENT
---------------------------------------------
- Registered under the name "fastoad.submodel.weight.mass.airframe.wing.BWBtest.2"
wing_mass = flexionF_mass + shearF_mass + ribs_mass + reinforcements_mass + 2ndParts_mass
...
@RegisterSubmodel(SERVICE_WING_MASS, "fastoad.submodel.weight.mass.airframe.wing.BWBtest.2")
class WingWeight(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        wing_area = inputs["data:geometry:wing:area"] # tube wing
        wing_areaBWB = inputs["data:geometry:wing:outer_area"] # BWB
        span = inputs["data:geometry:wing:span"]
        spanBWB = inputs["data:geometry:wing:span"] - inputs["data:geometry:fuselage:maximum_width"]
        ...
        temp_a11 = (
            5.922e-5
            * k_voil
            * ((max_nm / (l2_wing * toc_mean)) * (spanBWB / np.cos(sweep_25)) ** 2.0) ** 0.9
        )
        weight_a11 = k_a11 * temp_a11 + offset_a11

        # A12=Mass of the wing due to shear
        temp_a12 = 5.184e-4 * k_voil * (max_nm * spanBWB / np.cos(sweep_25)) ** 0.9
        weight_a12 = k_a12 * temp_a12 + offset_a12

        # A13=Mass of the wing due to the ribs
        temp_a13 = k_voil * (1.7009 * wing_areaBWB + 10 ** (-3) * max_nm)
