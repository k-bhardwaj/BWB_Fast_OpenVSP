...
@RegisterOpenMDAOSystem("fastoad.loop.wing_area.2", domain=ModelDomain.OTHER)
...
class _ComputeWingArea(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        #self.add_input("data:mission:sizing:fuel_block", val=np.nan, units="kg")
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        ) ** (1.0 / 1.5) ## Computes the maximum wing area needed for the proposed mission. 

        approach_speed = inputs["data:TLAR:approach_speed"]
        mlw = inputs["data:weight:aircraft:MLW"]
        max_CL = inputs["data:aerodynamics:aircraft:landing:CL_max"]
        wing_area_approach = 2 * mlw * g / ((approach_speed / 1.23) ** 2) / (1.225 * max_CL) # Wing area computed from landing constraint.
        #print("This is the computed wing_area",np.nanmax([wing_area_mission, wing_area_approach]))
        outputs["data:geometry:wing:area"] = np.nanmax([wing_area_mission, wing_area_approach])


class _ComputeWingAreaConstraints(om.ExplicitComponent): ## Adds geometric constraints, assuming the fuel is stored in the wings.
    def setup(self):
        self.add_input("data:weight:aircraft:sizing_block_fuel", val=np.nan, units="kg")
