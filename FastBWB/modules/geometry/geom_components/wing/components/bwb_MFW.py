...
class ComputeMFWBWB(om.ExplicitComponent):
    """Max fuel weight estimation
    Inputs: Wing area, Wing AR, wing root thickness-over-chord, wingtip ToC.
    Outputs: MFW"""
    ...
    def setup(self):
        ...
        self.add_output("data:weight:MFW", units="kg")

    def setup_partials(self):
        self.declare_partials("data:weight:MFW", "*", method="fd")
    ...
    def compute(self, inputs, outputs):
        ...
        mfw = 224 * (wing_area ** 1.5 * lambda_wing ** (-0.4)
                     * (0.6 * el_emp + 0.4 * el_ext)) + 1570 ## Extracted from A. Sgueglia's work.


        outputs["data:weight:MFW"] = mfw