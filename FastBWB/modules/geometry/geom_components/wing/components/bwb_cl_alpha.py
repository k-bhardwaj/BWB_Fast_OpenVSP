...
class ComputeCLalpha(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_output("data:aerodynamics:CL_alpha")

    def setup_partials(self):
        self.declare_partials("data:aerodynamics:CL_alpha", "*", method="fd")
    ...
    def compute(self, inputs, outputs):
        ...
        cl_alpha_wing = 2 * math.pi * lambda_wing_eff / (2 + math.sqrt(4 + lambda_wing_eff**2 * beta**2 / 0.95**2 * (
                1 + (math.tan(sweep_25 / 180. * math.pi))**2 / beta**2))) * (wing_area - l2_wing * width_max) / wing_area * fact_F

        outputs["data:aerodynamics:CL_alpha"] = cl_alpha_wing
